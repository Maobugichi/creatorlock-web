export function SummarySkeleton() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {[0, 1, 2].map((i) => (
        <div key={i} className="bg-surface border border-border rounded-2xl p-5 space-y-3">
          <div className="h-3.5 w-28 bg-elevated rounded-xl animate-pulse" />
          <div className="h-7 w-40 bg-elevated rounded-xl animate-pulse" />
        </div>
      ))}
    </div>
  );
}

export function AffiliateRowSkeleton() {
  return (
    <div className="flex items-center justify-between px-5 py-4 border-b border-border last:border-0">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div className="w-9 h-9 rounded-xl bg-elevated animate-pulse shrink-0" />
        <div className="space-y-2 flex-1 min-w-0">
          <div className="h-3.5 w-32 bg-elevated rounded-xl animate-pulse" />
          <div className="h-3 w-48 bg-elevated rounded-xl animate-pulse" />
        </div>
      </div>
      <div className="hidden sm:flex items-center gap-6 shrink-0 ml-4">
        <div className="h-3.5 w-10 bg-elevated rounded-xl animate-pulse" />
        <div className="h-3.5 w-20 bg-elevated rounded-xl animate-pulse" />
        <div className="h-6 w-16 bg-elevated rounded-xl animate-pulse" />
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-elevated rounded-lg animate-pulse" />
          <div className="h-7 w-16 bg-elevated rounded-lg animate-pulse" />
          <div className="h-7 w-20 bg-elevated rounded-lg animate-pulse" />
          <div className="w-8 h-8 bg-elevated rounded-lg animate-pulse" />
        </div>
      </div>
    </div>
  );
}