...
export function AffiliateActionSheet({ affiliate, onClose }: AffiliateActionSheetProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md rounded-t-2xl sm:rounded-2xl p-6 flex flex-col gap-5 bg-surface border border-border"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:text-surface-foreground transition-colors bg-elevated"
          aria-label="Close"
        >
        ...
        <div className="flex items-center gap-3 pr-8">
          <div className="w-11 h-11 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0">
            <span className="text-primary text-sm font-syne font-bold">{initials}</span>
          </div>
          <div className="min-w-0">
            <p className="text-surface-foreground text-sm font-semibold truncate">{affiliate.affiliate_name}</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-elevated border border-border rounded-xl p-3.5">
            <p className="text-muted-foreground text-xs uppercase tracking-wider mb-1">Conversions</p>
            <p className="text-surface-foreground text-lg font-mono font-semibold">{affiliate.total_conversions}</p>
            {affiliate.conversions_this_week > 0 && (
              <p className="text-status-positive text-xs font-medium mt-0.5">+{affiliate.conversions_this_week} this week</p>
            )}
          </div>
            ...
            {editingCommission ? (
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  autoFocus
                  className="w-14 bg-surface border border-primary/40 rounded-lg px-1.5 py-1 text-surface-foreground text-sm font-mono focus:outline-none focus:border-primary"
                />
                <button onClick={handleSaveCommission} disabled={isUpdating} className="text-status-positive" aria-label="Save">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </button>
                <button onClick={() => setEditingCommission(false)} className="text-muted-foreground" aria-label="Cancel">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              ...
              <div className="flex items-center gap-1.5">
                <p className="text-surface-foreground text-lg font-mono font-semibold">{affiliate.commission_percent}%</p>
                <button
                  onClick={() => {
        ...
        <div className="flex items-center justify-between bg-elevated border border-border rounded-xl p-3.5">
        ...
        <div className="flex flex-col gap-2.5 border-t border-border pt-5">
          <button
            ...
            className="w-full text-sm font-syne font-semibold px-4 py-3 rounded-xl transition-all bg-elevated text-surface-foreground border border-border hover:bg-border-strong disabled:opacity-50 disabled:pointer-events-none"
          >
            {isResending ? 'Resending…' : onCooldown ? `Resend available in ${formatCooldown(cooldownRemaining)}` : 'Resend Invite'}
          </button>

          {resendError && <p className="text-xs text-status-exception text-center">{resendError}</p>}
          ...
          {affiliate.total_conversions > 0 ? (
            ...
            <>
              <button
                ...
                disabled={isDeleting}
                className="w-full text-sm font-syne font-semibold px-4 py-3 rounded-xl transition-all bg-transparent text-status-exception/70 hover:text-status-exception hover:bg-status-exception/5 disabled:opacity-50"
              >
                {isDeleting ? 'Deleting…' : confirmingDelete ? 'Tap again to permanently delete' : 'Delete Affiliate'}
              </button>
              {deleteError && <p className="text-xs text-status-exception text-center mt-1">{deleteError}</p>}
            </>
          )}
        </div>
      </div>
