...
import {
  useAdminCreatorPayouts,
  useApproveCreatorPayout,
  useProcessCreatorPayout,
} from '../api/useAdminCreatorPayouts';
...
export function CreatorPayoutsTab() {
  ...
  const { data: payouts, isLoading, isError } = useAdminCreatorPayouts(
    statusFilter === 'all' ? undefined : statusFilter
  );
  const { approve, isPending: isApproving, error: approveError } = useApproveCreatorPayout();
  const { process, isPending: isProcessing, error: processError } = useProcessCreatorPayout();
    ...
    <div className="space-y-4">
      ...
      <div className="bg-surface border border-[var(--border)] rounded-2xl overflow-hidden">
        ...
        {!isLoading && !isError && payouts?.map((payout) => (
          <PayoutQueueRow
            key={payout.id}
            recipientName={payout.creator_name}
            recipientSub={payout.creator_email}
