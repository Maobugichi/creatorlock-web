...
export function BuyerRowItem({ buyer, selected, onSelect }: Props) {
  return (
    <div
      className={`flex items-center justify-between px-5 py-4 border-b border-[var(--border)] last:border-0 transition-colors ${
        selected ? 'bg-brand/[0.04]' : 'hover:bg-white/[0.015]'
      }`}
    >
      {/* Checkbox */}
      <input
        ...
        checked={selected}
        onChange={onSelect}
        className="w-3.5 h-3.5 rounded border-[var(--border)] accent-brand cursor-pointer shrink-0 mr-4"
