      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto bg-surface border border-border rounded-2xl shadow-xl z-50"
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-border">
              <span className="font-syne font-bold text-sm text-surface-foreground">Notifications</span>
              {count > 0 && (
                <button
                  onClick={() => markAllRead.mutate()}
                  disabled={markAllRead.isPending}
                  className="text-xs text-primary hover:text-primary-dark font-syne font-semibold transition-colors disabled:opacity-50"
                >
                  Mark all read
                </button>
              )}
              ...
              <div className="divide-y divide-border">
                {notifications.map((n) => {
                  const content = (
                    <div
                      className={[
                        "px-4 py-3 hover:bg-elevated transition-colors cursor-pointer",
                    ...
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-syne font-semibold text-xs text-surface-foreground">{n.title}</p>
    ...
    <header className="h-14 flex items-center justify-between px-4 md:px-6 border-b border-border bg-background flex-shrink-0">
      <div className="flex items-center gap-3">
        <button
          ...
          className="md:hidden w-9 h-9 rounded-xl flex items-center justify-center text-surface-foreground/70 hover:text-surface-foreground hover:bg-elevated transition-colors"
        >
          <List size={22} weight="bold" />
        </button>

        <h1 className="hidden md:block font-syne font-bold text-lg text-surface-foreground tracking-tight">
          {title}
        </h1>
      </div>
