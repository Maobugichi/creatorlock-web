...
import { useCreateCoupon } from '../_hooks/useCreateCoupons';
...
export function CreateCouponForm() {
  ...
  const handleSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    const payload: CreateCouponPayload = {
      ...
      discount_type: data.get('discount_type') as 'percent' | 'flat',
      discount_value: parseFloat(data.get('discount_value') as string),
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              id="discount_value" name="discount_value" type="number" required min={1} placeholder="20"
          ...
          <div className="space-y-1.5">
            ...
            <input
              id="max_uses" name="max_uses" type="number" min={1} placeholder="Unlimited"
              className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-[var(--muted)] font-mono focus:outline-none focus:border-brand/50 transition-colors"
            />
          </div>
