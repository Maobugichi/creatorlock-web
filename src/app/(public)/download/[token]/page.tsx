...
const redeemToken = cache(async (token: string): Promise<DownloadResult> => {
  try {
    ...
    return (
      <main className="min-h-screen bg-[var(--bg)] text-white flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <Link href="/" className="inline-block bg-brand hover:bg-brand-dark active:scale-[0.98] text-white font-syne font-semibold rounded-xl py-3 px-8 transition-all duration-150">
            Back to CreatorLock
          </Link>
        </div>
      </main>
      ...
      <div className="max-w-lg w-full space-y-6">
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-[var(--muted)] text-sm mt-1">Click below to download your file{result.downloads.length > 1 ? 's' : ''}</p>
          </div>
        </div>
        ...
        <div className="bg-surface border border-[var(--border)] rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[var(--border)]">
            <p className="text-xs text-[var(--muted)] uppercase tracking-widest">
              {result.downloads.length} {result.downloads.length === 1 ? 'file' : 'files'} ready to download
            </p>
          </div>
          <ul className="divide-y divide-[var(--border)]">
            {result.downloads.map((file, index) => (
